// Ported from Assets/PixelPlanets/Shaders/NoAtmosphere.shader
import { commonNoiseFunctions, commonUniformDeclarations } from '../glsl/common'

export const noAtmosphereFragmentShader = /* glsl */ `
varying vec2 vUv;
...
void main() {
  ...
  uv = rot(uv, uRotation);
  float f1 = fbm(uv);
  d_light += fbm(uv * uSize + f1 + vec2(uPlanetTime * uTimeSpeed, 0.0)) * 0.3;
