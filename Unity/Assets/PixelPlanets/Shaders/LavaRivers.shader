// Toon version — smooth lava river layer
Shader "PixelPlanets/LavaRivers"
{
    Properties
    {
        ...
        _RiverCutoff  ("River Cutoff",  Range(0,1))= 0.4
        _Size         ("Size",          Float)     = 50
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float  _Rotation, _TimeSpeed, _LightBorder1, _LightBorder2, _RiverCutoff;
            float4 _LightOrigin;
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                float f1        = fbm(uv*_Size + float2(_PlanetTime*_TimeSpeed, 0));
                float river_fbm = fbm(uv + f1*2.5);

                float dl = pow(d_light,2)*0.4;
                dl -= dl * river_fbm;

                float t1 = smoothstep(_LightBorder1-0.025, _LightBorder1+0.025, dl);
                float t2 = smoothstep(_LightBorder2-0.025, _LightBorder2+0.025, dl);
                float4 col = lerp(_Colors[0], _Colors[1], t1);
                col        = lerp(col,        _Colors[2], t2);

                // Smooth river edge — glowing lava veins
                float riverAlpha = smoothstep(_RiverCutoff-0.05, _RiverCutoff+0.05, river_fbm);
                return float4(col.rgb, riverAlpha*a*col.a);
            }
            ENDCG
        }
    }
